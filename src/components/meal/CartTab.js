...
//   const handleEnterKey = (e) => {
//     if (e.key === 'Enter') fetchReservation();
//   };
...
//       orderDate: formattedDate,
//       mealType: meal?.toUpperCase() || '',
//       items: items.map((i) => ({ foodId: i.productId, portions: i.quantity })),
//     };
//     try {
//       await axios.post(`${API_URL}/food-orders/place`, payload);
...
const CartTab = () => {
  ...
  // ✅ If popup is open, render ONLY the popup — nothing else
  if (showPopup) {
    return (
      <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
        <div className="bg-white w-11/12 max-w-lg p-6 rounded-xl shadow-lg relative">
          ...
          {reservations.length > 0 && (
            <div className="mb-4">
              <label className="block text-sm font-semibold mb-1">Reservation</label>
              <select
                className="w-full border rounded p-2"
                value={reservation?.reservationId ?? ''}
                onChange={(e) => {
  ...
  // ✅ Default cart layout when popup is not open
  return (
    <>
      {msg && (
        <div
