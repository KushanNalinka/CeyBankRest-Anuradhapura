...
const CartItem = ({ data }) => {
  const { updateStoreQuantity } = useStore(); // ✅ Changed function name
  const { productId, quantity } = data;
  const [detail, setDetail] = useState({});
  ...
  return (
    <div className='flex justify-between items-center bg-white text-black p-2 gap-5 mb-3 shadow-md rounded-xl w-90 h-24'>
      ...
      <div className='flex items-center gap-2'>
        <button className='bg-[#28245F] rounded-full w-6 h-6 text-white font-black' onClick={() => updateStoreQuantity(productId, quantity - 1)}>-</button>
        <span className='font-bold'>{quantity}</span>
        <button className='bg-[#28245F] rounded-full w-6 h-6 text-white font-black' onClick={() => updateStoreQuantity(productId, quantity + 1)}>+</button>
      </div>
    </div>
