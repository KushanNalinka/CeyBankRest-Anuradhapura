...
//       <h2 className="text-2xl font-bold mb-4">Items for Store Requisition ID: {id}</h2>
//       <button
//         onClick={() => navigate(-1)}
...
//     axios
//       .put(`${API_URL}/v1/store-requisitions/approve-status/${id}`)
//       .then(() => {
...
//                     item.approvedQuantity > 0 ? 'bg-green-500 hover:bg-green-600' : 'bg-yellow-500 hover:bg-yellow-600'
//                   }`}
//                 >
//                   Approve
//                 </button>
...
//                         setShowPopup(true); // keep showFinalizePopup open
//                       }}
//                     >
//                       <FaEdit />
//                     </button>
...
export default function StoreRequisitionItems() {
  ...
  const handleQuantityChange = (id, value) => {
    setItems(prev =>
      prev.map(item =>
        item.id === id ? { ...item, approvedQuantity: Number(value) } : item
      )
    );
  ...
  const finalizeStoreRequisition = () => {
    const itemsToApprove = items.filter(item => selectedItems[item.id]);
    const payload = {
      approvals: itemsToApprove.map(item => ({
        itemId: item.id,
        approvedQuantity: item.approvedQuantity,
      })),
    };
    axios
      .put(`${API_URL}/v1/store-requisitions/approve-itemslist`, payload)
      .then(() => {
        setShowFinalizePopup(false);
        navigate(-1);
      })
